...
case "${1:--help}" in
     '--phpunit'|'-pu')
          ...
          docker-compose exec -T php composer phpunit tests/Unit

          echo "${COLOUR_GREEN}Setup Test Database For Functional Tests...${RESET}"
          docker-compose exec -T php bin/console --env=test --no-interaction doctrine:database:drop --if-exists --force
          docker-compose exec -T php bin/console --env=test doctrine:database:create
          docker-compose exec -T php bin/console --env=test --no-interaction doctrine:migration:migrate

          echo "${COLOUR_GREEN}Running Functional Tests...${RESET}"
          docker-compose exec -T php composer phpunit tests/Functional

          echo "${COLOUR_GREEN}Dropping Test Database...${RESET}"
          docker-compose exec -T php bin/console --env=test --no-interaction doctrine:database:drop --if-exists --force
          ;;
     '--fixer'|'-cs')
          docker-compose exec -T php composer cs-fixer
          ;;
     '--phpstan'|'-ps')
